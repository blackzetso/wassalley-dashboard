            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <div class="d-sm-flex align-items-sm-center">
                        ...
                        @elseif($order['order_status'] == 'failed')
                            <span class="badge badge-soft-danger ml-2 ml-sm-3 text-capitalize">
                                <span
                                    class="legend-indicator text-capitalize bg-danger"></span>{{ __('messages.payment') }}
                                {{ __('messages.failed') }}
                            </span>
                        @else
                            <span class="badge badge-soft-danger ml-2 ml-sm-3 text-capitalize">
                    ...
                    <div class="d-flex px-0">
                        <div>
                            <p class="mb-0 small font-weight-bold text-dark">تاريخ الطلب</p>
                            <span class="">
                        ...
                        <div class="mx-2">
                            <p class="mb-0 small font-weight-bold text-dark">تاريخ التوصيل</p>
                            <span class="">
                                <i class="tio-date-range"></i>
                                {{ date('d M Y ' . config('timeformat'), strtotime($order['delivery_date'])) }}
                            </span>
                        </div>
                        <div>
                            <p class="mb-0 small font-weight-bold text-dark"> وقت التوصيل</p>
                            <span class="">
                                <i class="tio-time"></i>
                                {{ date('h:i a ' . config('timeformat'), strtotime($order['delivery_time'])) }}
                            </span>
                        </div>
                    </div>
                    <div class="mt-2">
                        ...
                        <div class="hs-unfold ml-1">
                            <h5>
                                ...
                                @foreach ($departments as $department)
                                    <label class="badge badge-danger">{{ $department['name'] }}</label>
                                @endforeach
                            </h5>
                        </div>
                        @php
                        ...
                        <div class="hs-unfold ml-1">
                            <h5>
                                <button class="btn btn-xs btn-secondary" data-toggle="modal" data-target="#locationModal"><i
                                        class="tio-map"></i> {{ __('messages.show_locations_on_map') }}</button>
                                @if ($order->payment_method != 'cash_on_delivery' && $order->payment_status == 'paid' && $order->order_status != 'refunded')
                                    <button class="btn btn-xs btn-danger"
                                        onclick="route_alert('{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'refunded']) }}','{{ __('messages.you_want_to_refund_this_order', ['amount' => $refund_amount . ' ' . \App\CentralLogics\Helpers::currency_code()]) }}', '{{ __('messages.are_you_sure_want_to_refund') }}')"><i
                                            class="tio-money"></i> {{ __('messages.refund_this_order') }}</button>
                                @endif
                            </h5>
                        </div>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                        ...
                                        <a class="dropdown-item {{ $order['order_status'] == 'handover' ? 'active' : '' }}"
                                            onclick="route_alert('{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'handover']) }}','Change status to handover ?')"
                                            href="javascript:">{{ __('messages.handover') }}</a>
                                        <a class="dropdown-item {{ $order['order_status'] == 'picked_up' ? 'active' : '' }}"
                                            onclick="route_alert('{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'picked_up']) }}','Change status to out for delivery ?')"
                                            href="javascript:">{{ __('messages.out_for_delivery') }}</a>
                                        <a class="dropdown-item {{ $order['order_status'] == 'delivered' ? 'active' : '' }}"
                                            onclick="route_alert('{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'delivered']) }}','Change status to delivered (payment status will be paid if not)?')"
                                    ...
                                    <form id="search-form" class="header-item">
                                        ...
                                        <div class="input-group input-group-merge input-group-flush">
                                            ...
                                            <input id="datatableSearch" type="search"
                                                value="{{ isset($keyword) ? $keyword : '' }}" name="search"
                                                class="form-control" placeholder="Search here" aria-label="Search here">
                                        </div>
                                        <!-- End Search -->
                                    </form>
                                ...
                                <div class="media">
                                    ...
                                    @else
                                        <a class="avatar avatar-xl mr-3"
                                            href="{{ route('admin.product.view', $detail->product['id']) }}">
                                            <img class="img-fluid"
                                                src="{{ asset('storage/app/public/product') }}/{{ $detail->product['image'] }}"
                                                onerror="this.src='{{ asset('public/assets/admin/img/160x160/img2.jpg') }}'"
                                                alt="Image Description">
                                                ...
                                                @php
                                                    ...
                                                    if (isset($detail->product['category_ids'])) {
                                                        if (is_string($detail->product['category_ids'])) {
                                                            foreach (json_decode($detail->product['category_ids']) as $productCategory) {
                                                                $productCategories[] = [
                                                                    'id' => $productCategory->id,
                                                                ];
                                                            }
                                                        } else {
                                                            $productCategories = $detail->product['category_ids'];
                                                        }
                                                    }
                                                @endphp
                                                <hr>
                        ...
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-sm-right">
                                    <dt class="col-sm-6">{{ __('messages.items') }}
                                        {{ __('messages.price') }}:
                                    </dt>
                                    <dd class="col-sm-6">
                                ...
                                @foreach ($deliveryMen as $dm)
                                    <li class="list-group-item">
                                        ...
                                        @if ($order->delivery_man && $dm->id === $order->delivery_man->id)
                                            <a href="#"
                                                class="btn btn-dark btn-xs float-right disabled">{{ __('messages.current') }}</a>
                                        @else
...
@push('script_2')
    <script>
        ...
        function removeFromCart(key) {
            Swal.fire({
                ...
                if (result.value) {
                    $.post('{{ route('admin.orders.remove-from-cart') }}', {
                        _token: '{{ csrf_token() }}',
                        key: key,
                        order_id: '{{ $order->id }}'
                    }, function(data) {
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ \App\Model\BusinessSetting::where('key', 'map_api_key')->first()->value }}&v=3.45.8">
    </script>
    <script>
