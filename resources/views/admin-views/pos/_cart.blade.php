        ...
        @if(session()->has('cart') && count( session()->get('cart')) > 0)
            ...
            @foreach(session()->get('cart') as $key => $cartItem)
                ...
                @if(is_array($cartItem))
                    ...
                    $addon_price += $cartItem['addon_price'] * $cartItem['quantity'];
                    ...
                    <tr>
                        ...
                        <td class="align-items-center text-center">
                            <input type="number" data-key="{{$key}}" style="width:50px;text-align: center;"
                                   value="{{$cartItem['quantity']}}" step="0.10" min="0.1"
...
$delivery_charge = session()->get('cart')['delivery_charge'] ?? 0;
...
<div class="box p-3">
    <dl class="row text-sm-right">
        ...
        <dt class="col-sm-6">{{\App\CentralLogics\translate('product')}} {{\App\CentralLogics\translate('discount')}}:
        </dt>
        <dd class="col-sm-6 text-right">
            - {{round($discount_amount,2) . ' ' . \App\CentralLogics\Helpers::currency_symbol()}}</dd>

        <dt class="col-sm-6">{{\App\CentralLogics\translate('extra')}} {{\App\CentralLogics\translate('discount')}}:
        </dt>
        <dd class="col-sm-6 text-right">
            <button class="btn btn-sm" type="button" data-toggle="modal" data-target="#add-discount"><i
                        class="tio-edit"></i></button>
            - {{$extra_discount . ' ' . \App\CentralLogics\Helpers::currency_symbol()}}</dd>
        <dt class="col-sm-6"> {{\App\CentralLogics\translate('delivery fee')}} :</dt>
        <dd class="col-sm-6 text-right">
            <button class="btn btn-sm" type="button" data-toggle="modal" data-target="#add-delivery-fee"><i
                        class="tio-edit"></i>
            </button> {{$delivery_charge . ' ' . \App\CentralLogics\Helpers::currency_symbol()}}</dd>
...
<div class="modal fade" id="add-delivery-fee" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{\App\CentralLogics\translate('update delivery fee')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{route('admin.pos.update_delivery_fee')}}" method="post" class="row">
                    @csrf
                    <div class="form-group col-sm-6">
                        <label for="">{{\App\CentralLogics\translate('delivery fee')}}</label>
                        <input type="number" class="form-control" name="delivery_charge"
                               value="{{ session()->get('cart')['delivery_charge'] ?? 0 }}">
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('admin.pos.order')}}" id='order_place' method="post" class="row">
                    @csrf
                    <div class="form-group col-12">
                        <label class="input-label" for="">amount({{\App\CentralLogics\Helpers::currency_symbol()}}
                            )</label>
